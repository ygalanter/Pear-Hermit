...
class MySettings extends SettingsComponent {
  renderCollapsable({ key, label, content }) {
    const {
      settings: { [key]: openSetting = "false" },
      settingsStorage,
    } = this.props;
  ...
  renderColorSelect({ title, settingsKey }) {
    return [<ColorSelect settingsKey={settingsKey} colors={colors} />];
  }
    ...
    return (
      <Page>
        ...
        >
          ...
          <Toggle
            settingsKey="transparent_hands"
            label="Transparent hour/minute hands"
            checked="on"
          />
        </Section>
        ...
        >
          <Text italic>
            If you like this clockface and would like to see it further
            developed as well as other wonderful Fitbit apps and faces created,
            please know - I run on coffee. It's an essential fuel for
            inspiration and creativity. So feel free to donate by tapping the
            link below so I won't run out of fuel :) Thanks!
          </Text>
